...
class NN_generate_data(object):
    
    def init(self):
        pass
        
    def random_state(self, time_step, p_c = 1.5, v_c = 2.0, curr_state = None):
        self.ranges = np.array([
        p_c, p_c, p_c, v_c * time_step, v_c * time_step, v_c * time_step, 
        np.pi, np.pi, np.pi,
        np.pi, np.pi, np.pi, 1])
        state = np.array([])
        
        if curr_state != None:
            x, y, z, x_p, y_p, z_p, R11, R12, R13, R21, R22, R23, R31, R32, R33, p, q, r, z_pp = curr_state
        else:
            x, y, z, x_p, y_p, z_p = (0, 0, 0, 0, 0, 0)
            
        for i in range(6):
            state = np.append(state, np.random.uniform(-self.ranges[i], self.ranges[i]))
        x_n, y_n, z_n, x_p_n, y_p_n, z_p_n = state
        R11, R12, R13, R21, R22, R23, R31, R32, R33 = (1, 0, 0, 0, 1, 0, 0, 0, 1)
        p, q, r, z_pp = (0, 0, 0, 0)
        
        '''
        x_p_d = x_p * (random.random() * 0.4 - 0.2)
        y_p_d = y_p * (random.random() * 0.4 - 0.2)
        z_p_d = z_p * (random.random() * 0.4 - 0.2)
        ...
        x_d = (x_p + x_p_n / 2.0) * time_step + (random.random() - 0.5) * 1.0
        y_d = (y_p + y_p_n / 2.0) * time_step + (random.random() - 0.5) * 1.0
        z_d = (z_p + z_p_n / 2.0) * time_step + (random.random() - 0.5) * 1.0
        
        return np.array([x + x_d, y + y_d, z + z_d, x_p + x_p_n, y_p + y_p_n, z_p + z_p_n, R11, R12, R13, R21, R22, R23, R31, R32, R33, p, q, r, z_pp])
    
    def generate_data(self, p_c = 2.0, v_c = 1.5, show = True):
        ...
        state_init = self.random_state(time_step, p_c, v_c)
        desire_state = self.random_state(time_step, p_c, v_c, state_init)
        ...
        for tick in np.linspace(0, sim_time, (sim_time / time_per_tick + 1)):
            ...
            if tick % outer_loop_cycle == 0:
                ...
                if count == target_count:
                    count = 0
                    states = []
                    desire_state = self.random_state(time_step, p_c, v_c, state)
